...
dfil = np.loadtxt('dmat_fil_comp.dat',dtype=np.float64)   # check matrix
dfil = dfil[:,1:]/10.0                                    # make it nm from Ang

rho_f = np.loadtxt("rgyr_f.dat")[:,1]
rho_f = myclusters2.size_indep_rho(dfil,rgyr_f)
nfeatures = 897
...
ndata = dfil.shape[1]  # check matrix 
...
for i  in [0,1,2,3,4]:
    estimator = myclusters2.density_peaks(cutoff="auto",percent=perc[i],metric=spacedef, kernel="gaussian",D = dfil)  #check matrix
    ...
    met1[i,0]  =  metrics.silhouette_score( dfil , newlabels , metric = spacedef) / Cor1     #check matrix 
    eval1 = mymetrics2.cluster_eval(metric = spacedef, clusters = newlabels, D = dfil )       #check matrix
    ...
    dens_score = DBCV.DBCV(clusters=newlabels,metric=spacedef,NF=nfeatures,D=dfil)
    met1[i,5] = dens_score.calc_score(meas='kernel')    
    
    print "**** End of Run %d ********"  % (i+1)

np.savetxt("R18-MET_rmsd_f.DP",met1,fmt="%6.4f")


for i  in [0,1,2,3,4]:
    estimator = myclusters2.density_peaks(cutoff="auto",percent=perc[i],metric=spacedef, kernel="gaussian",D = rho_f)  #check matrix
    rho,delta = estimator.decision_graph()
    N = np.vstack((rho,delta))  #save this stack
    np.savetxt( outf[i], N, fmt="%5.3f")
    ...
    met1[i,0]  =  metrics.silhouette_score( rho_f, newlabels , metric = spacedef) / Cor1     #check matrix 
    eval1 = mymetrics2.cluster_eval(metric = spacedef, clusters = newlabels, D = rho_f )       #check matrix
    ...
    met1[i,3] = met1[i,3]/Cor1
    met1[i,4] = met1[i,4]/Cor2
    dens_score = DBCV.DBCV(clusters=newlabels,metric=spacedef,NF=nfeatures,D=rho_f)
    met1[i,5] = dens_score.calc_score(meas='kernel')    
    
    print "**** End of Run %d ********"  % (i+1)

np.savetxt("R18-MET_rho_f.DP",met1,fmt="%6.4f")
